...
class Pizza {
    constructor(name, description, prices) {
        this.name = name;
        this.description = description;
        this.prices = prices;
    }
    getPizza(size) {
        return `${this.name} ${this.description} ${size} ${this.prices[size]}`;
    }
}
//////////////////////////////////////////
let margarita = {
...
let pizza1 = new Pizza('Margarita', 'Tomato and mozzarella', margarita);
let pizza2 = new Pizza('Capriccosa', 'Tomato, mozzarella, olives, ham, boiled egg', capricciosa);
let pizza3 = new Pizza('Diavola', 'Tomato, mozzarella, onion, chili pepper', diavola);
let pizza4 = new Pizza('Pascolo', 'Mozzarella, stracchino, prosciutto, rocket', pascolo);
let pizza5 = new Pizza('Quattro-stagioni', 'Tomato, mozzarella, mushrooms, artichokes', quattroStagioni);
let pizza6 = new Pizza('Giardiniera', 'Mozzarella, courgettes, aubergines, peppers', giardiniera);
...
const selPizzaSize = document.querySelectorAll('.size');
const btnOrder = document.querySelectorAll('.order-btn');
const orderContent = document.querySelector('#order-content');
const orderSum = document.querySelector('#order-sum');
let sum = 0;

selPizzaSize.forEach(e => {
        let price;
        pizzas.forEach(pizza => {
            if(pizza.name == e.id)
                price = pizza.prices[e.value];
        })
        e.previousElementSibling.innerHTML = price + ' $';
});

selPizzaSize.forEach(select => {
    select.addEventListener('change', (e) =>{
        let price;
        pizzas.forEach(pizza => {
            if(pizza.name == e.target.id)
                price = pizza.prices[e.target.value];
        })
        console.log(e.target.previousSibling);
        e.target.previousElementSibling.innerHTML = price + ' $';
    });
});

btnOrder.forEach(btn => {
    btn.addEventListener('click', e => {
        let select = document.querySelector(`#${e.target.id}`);
        sum += Number(select.previousElementSibling.innerHTML.slice(0, -1));
        orderContent.innerHTML += `
        <tr>
            <td>${e.target.id}</td>
            <td>${select.value}</td>
            <td>${select.previousElementSibling.innerHTML}</td>
        </tr>
        `;
